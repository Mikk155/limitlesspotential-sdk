...
void R_Explosion( const Vector& pos, int model, float scale, float framerate, int flags )
{
    if( scale != 0 )
    {
        ...
        {
            if( dlight_t* yellowLight = gHUD.m_Dlight.AllocDLight( Explosions ); yellowLight != nullptr )
            {
            ...
            if( dlight_t* orangeLight = gHUD.m_Dlight.AllocDLight( Explosions ); orangeLight != nullptr )
            {
