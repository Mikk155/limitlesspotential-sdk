...
void CStomp::Think()
{
    ...
    if( tr.pHit && tr.pHit != pev->owner )
    {
        ...
        if( pEntity )
            pEntity->TakeDamage( this, owner, g_Skill.GetValue( "gargantua_dmg_stomp"sv, 100, this ), DMG_SONIC );
    }
...
void CGargantua::OnCreate()
{
    ...
    pev->health = g_Skill.GetValue( "gargantua_health"sv, 1000, this );
...
void CGargantua::FlameUpdate()
{
    ...
    for( i = 0; i < 2; i++ )
    {
        if( m_pFlame[i] )
        {
            ...
            // RadiusDamage(trace.vecEndPos, this, this, g_Skill.GetValue( "gargantua_dmg_fire"sv, 5, this ), DMG_BURN, Classify());
            FlameDamage( vecStart, trace.vecEndPos, this, this, g_Skill.GetValue( "gargantua_dmg_fire"sv, 5, this ), DMG_BURN, Classify() );
...
void CGargantua::HandleAnimEvent( MonsterEvent_t* pEvent )
{
    ...
    {
        // HACKHACK!!!
        CBaseEntity* pHurt = GargantuaCheckTraceHullAttack( GARG_ATTACKDIST + 10.0, g_Skill.GetValue( "gargantua_dmg_slash"sv, 30, this ), DMG_SLASH );
        if( pHurt )
        {
