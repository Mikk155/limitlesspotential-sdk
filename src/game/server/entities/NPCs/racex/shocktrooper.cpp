...
void CShockTrooper::OnCreate()
{
    ...
    pev->health = g_Skill.GetValue( "shocktrooper_health"sv, 200, this );
...
bool CShockTrooper::CheckRangeAttack2( float flDot, float flDist )
{
    ...
    else
    {
        ...
        // estimate position
        if( HasConditions( bits_COND_SEE_ENEMY ) )
            vecTarget = vecTarget + ( ( vecTarget - pev->origin ).Length() / g_Skill.GetValue( "shocktrooper_gspeed"sv, 800, this ) ) * m_hEnemy->pev->velocity;
    }
    ...
    {
        Vector vecToss = VecCheckThrow( this, GetGunPosition(), vecTarget, g_Skill.GetValue( "shocktrooper_gspeed"sv, 800, this ), 0.5 );
...
void CShockTrooper::HandleAnimEvent( MonsterEvent_t* pEvent )
{
    ...
    case STROOPER_AE_KICK:
    {
        ...
        if( pHurt )
        {
            ...
            pHurt->TakeDamage( this, this, g_Skill.GetValue( "shocktrooper_kick"sv, 10, this ), DMG_CLUB );
        }
    }
    break;
...
bool CShockTrooper::Spawn()
{
    ...
    m_cClipSize = g_Skill.GetValue( "shocktrooper_maxcharge"sv, 8, this );
...
{
    if( gpGlobals->time - m_flLastChargeTime >= g_Skill.GetValue( "shocktrooper_rchgspeed"sv, 1, this ) )
    {
