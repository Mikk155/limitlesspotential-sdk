...
SpawnAction CFuncPlatRot::Spawn()
{
    CFuncPlat::Spawn();
    SetupRotation();
...
SpawnAction CSpriteTrain::Spawn()
{
    Precache();
    if( pev->speed == 0 )
        pev->speed = 100;
...
void CFuncTrackTrain::Use( CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, UseValue value )
{
    ...
    {
        float delta = ( value.Float.has_value() ? value.Float.value() : 0 );
...
SpawnAction CFuncTrackTrain::Spawn()
{
    if( pev->speed == 0 )
        m_speed = 100;
    else
...
SpawnAction CFuncTrainControls::Spawn()
{
    pev->solid = SOLID_NOT;
    pev->movetype = MOVETYPE_NONE;
    SetModel( STRING( pev->model ) );
...
SpawnAction CFuncTrackChange::Spawn()
{
    Setup();
