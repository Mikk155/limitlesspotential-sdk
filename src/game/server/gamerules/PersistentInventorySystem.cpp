...
void PersistentInventorySystem::InitializeFromPlayers()
{
    ...
    for( int i = 1; i <= gpGlobals->maxClients; ++i )
    {
        ...
        inventory.PlayerId = player->GetSteamID()->steamID64;
...
bool PersistentInventorySystem::TryApplyToPlayer( CBasePlayer* player )
{
    ...
    const float gracePeriod = g_cfg.GetValue<float>( "coop_persistent_inventory_grace_period"sv );

    // Grace period ended, can't restore.
    if( static_cast<int>( gracePeriod ) != -1 && gpGlobals->time >= ( player->m_ConnectTime + std::max( 0.f, gracePeriod ) ) )
    {
        return false;
    }

    const uint64_t playerId = player->GetSteamID()->steamID64;
